...
fn visit_dirs(dir: &Path, cb: &Fn(&DirEntry, &mut Vec<usize>) -> io::Result<()>, map: &mut Vec<usize>) -> io::Result<()> {
	dbg!(&dir);
    if dir.is_dir() {
...
fn check_pub(file: &DirEntry, map: &mut Vec<usize>) -> io::Result<()> {
	...
	loop {
		match pub_found(&b, c) {
			Some(true) => {	if pub_is_needless(&mut b, c, file) {
			    map.push(c);
			}}
			Some(_) => {},
			None => break
...
fn pub_is_needless(b: &mut Vec<u8>, c: usize, file: &DirEntry) -> bool {
    ...
    let out = if out.stdout.is_empty() {out.stderr} else {out.stdout};
...
fn pub_found(v: &Vec<u8>, c: usize) -> Option<bool> {
...
fn main() {
	...
	visit_dirs(std::path::Path::new("./src"), &check_pub, &mut indexes);
	dbg!(indexes);
}
